...
const PROVIDERS = ["local", "cryptocompare"];

function getProviderName() {
    return process.env.COINE_PROVIDER || "local";
}

function withCoinConfig(nextConfig = {}) {
    const config = merge({ coin: { provider: getProviderName() } }, nextConfig);
    const { coin } = config;
    const { provider } = coin;
    ...
    if (coin.updateTSConfig !== false) {
        const tsconfigPath = path.join(
            process.cwd(),
            coin.tsconfigPath || "tsconfig.json"
        );
